...
layout = [  [sg.Text('IGNOU PERCENTAGE CALCULATOR (BA/BCOM/BDP/MEG)', size=(60, 1), justification='center', font=('ubuntu', 15), relief=sg.RELIEF_RIDGE)],
            [sg.Text('Enter your Enrollment no. (only for BA/BDP/BCOM/MEG programmes):')],      
            [sg.In(size=(20,1), key='enrollmentNo')],      
            [sg.Txt('Choose your programme code (BA/BDP/BCOM/MEG):')],
            [sg.InputCombo(('BA', 'BDP', 'BCOM', 'MEG'), key='programmeCode', size=(6, 1), default_value='BA')],      
...
window = sg.Window('IGNOU PERCENTAGE CALCULATOR (BA/BCOM/BDP/MEG) -- v1.1alpha', layout )      
...
while True:      
    ...
    if event is not None:      
        try:
            ...
            result_url = ""
            if programmeCode == 'BA' or 'BDP' or 'BCOM':
                result_url = 'https://gradecard.ignou.ac.in/gradecardB/Result.asp?eno=' + enrollmentNo + '&program=' + programmeCode + '&hidden_submit=OK'
            if programmeCode == 'MEG':
                result_url = 'https://gradecard.ignou.ac.in/gradecardR/Result.asp?eno=' + enrollmentNo + '&program=' + programmeCode + '&hidden_submit=OK'
            print (result_url)
